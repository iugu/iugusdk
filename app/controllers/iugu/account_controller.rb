class Iugu::AccountController < Iugu::AccountSettingsController
  ...
  before_filter(:only => [:destroy, :cancel_destruction, :update]) { |c| c.must_be [:owner, :administrator], :id }
  before_filter(:only => [:payment_history, :generate_new_token]) { |c| c.must_be [:owner, :administrator], :account_id }
